...
export default function CustomerList() {
  ...
  const openEdit = (c) => {
    setEditing(c);
    setForm({ name: c.name || '', address: c.address || '', phone: c.phone || '', str_number: c.str_number || '', ntn: c.ntn || '', is_active: c.is_active ?? true });
    setError('');
    setShowModal(true);
  };
    ...
    <Layout>
      <div className="animate-fadeIn min-h-screen bg-[#f4f5f9] p-6">
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-gray-400">
              <svg className="w-14 h-14 mb-3 text-gray-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M17 20h5v-1a4 4 0 00-5.197-3.773M9 20H4v-1a4 4 0 015.197-3.773M15 7a4 4 0 11-8 0 4 4 0 018 0zm6 4a3 3 0 11-6 0 3 3 0 016 0zm-18 0a3 3 0 116 0 3 3 0 01-6 0z" />
              </svg>
              <p className="font-semibold text-gray-500">No customers found</p>
              <p className="text-sm mt-1">{search ? 'Try a different search term.' : 'Click "Add Customer" to get started.'}</p>
            </div>
          ) : (
                ...
                <tbody className="divide-y divide-gray-50">
                  {filtered.map(c => (
                    <tr key={c.id} className="hover:bg-gray-50/60 transition-colors">
                      ...
                      <td className="px-4 py-3 text-gray-600 whitespace-nowrap">{c.str_number || '—'}</td>
                      <td className="px-4 py-3 text-gray-600 whitespace-nowrap">{c.ntn || '—'}</td>
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        <div className="flex items-center gap-2">
                          <button onClick={() => openEdit(c)} className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium text-violet-700 bg-violet-50 hover:bg-violet-100 transition-colors">
                            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                            Edit
                          </button>
                          <button onClick={() => handleDelete(c)} className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium text-red-600 bg-red-50 hover:bg-red-100 transition-colors">
        ...
        {!loading && filtered.length > 0 && (
          <p className="text-xs text-gray-400 mt-3 px-1">{filtered.length} customer{filtered.length !== 1 ? 's' : ''} found</p>
        )}
          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
              <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                <h3 className="font-bold text-gray-900 text-lg">{editing ? 'Edit' : 'Add'} Customer</h3>
              ...
              <div className="p-6 space-y-4">
                ...
                <div>
                  ...
                  <input className={inp} placeholder="Customer name" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} />
                </div>
                <div>
